...
use num::Complex;
mod parse_args;
use parse_args::{parse_pair, parse_complex};
use rayon::prelude::*;
...
fn main() {
    ...
    let mut strategy = 'p';
    if args.len() == 6 {
        strategy = args[5].chars().next().unwrap();
    }

    let mut pixels = vec![0; bounds.0 * bounds.1];

    if strategy == 'p' {
        println!("Using parallel cross");
        parallel_render(&mut pixels, bounds, upper_left, lower_right);
    } else if strategy == 'r' {
        println!("Using rayon");
        parallel_rayon(&mut pixels, bounds, upper_left, lower_right);
    } 
...
fn parallel_rayon(pixels: &mut [u8],
                   bounds: (usize, usize),
